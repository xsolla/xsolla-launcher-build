...
import './index.css';
import {View, Title, ArrowButton, Caption, Subtitle} from '../../elements';
    ...
    return (
      <View className={`last-update ${this.className}`}>
        ...
        <View className="lu__games">
          <View className="lu__lenta" style={{left: `calc(-${pers}% - ${px}px)`}}>
  ...
  renderNewsBlock(n) {
    ...
    return (
      <View className="lu__game" key={uuidv4()} style={{direction: this.props.pageDirection}}>
        <img
          onClick={() => {
            window.openOneNews(n.id);
          }}
          className="lu__game__img"
          src={n.image}
          alt=""
        />
